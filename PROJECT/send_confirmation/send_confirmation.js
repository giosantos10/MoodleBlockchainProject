...
function attachMosaic() {
    nem.com.requests.namespace.mosaicDefinitions(endpoint, $("h3").text()).then(function(res) {
        ...
        var neededDefinition = nem.utils.helpers.searchMosaicDefinitionArray(res.data, ["submittedplace"]);

        // Get full name of mosaic to use as object key
        var fullMosaicName  = $("h3").text() + ':' + "submittedplace";
        ...
        var mosaicAttachment = nem.model.objects.create("mosaicAttachment")($("h3").text(), "submittedplace", quantity);
...
function send() {
    ...
    if(!$("#privatekey").val() || !$("#teacher_address").val()) return alert('missing parameter' + ("#privateKey").val());
    if (!nem.model.address.isValid(nem.model.address.clean($("#teacher_address").val()))) return alert('Invalid recipent address !');
    ...
    transferTransaction.recipient = nem.model.address.clean($("#teacher_address").val());
